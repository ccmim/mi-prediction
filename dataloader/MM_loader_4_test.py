...
class MM(data.Dataset):
    ...
    def __init__(self,
        ...
        for idx, ID in enumerate(ids_set.values):
            ...
            if len(img_21015) >= 1:
                ...
                # Automatic: mtd sex[24], dbpa[30], sbpa[31], ss[33], ads[34], bmi[36], age[38], hba1c[40], chol[41], glucose[43]
    ...
    def __getitem__(self, index):
        ...
        # sax image. NOT needed during testing
        sax = np.zeros((15, 128, 128))

        return fundus_image, torch.FloatTensor(sax), torch.FloatTensor(self.mtdt[index]), self.img_names[index] # index  # torch.FloatTensor(self.mtdt[index])
